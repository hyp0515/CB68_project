...
class Model:
    ...
    def interp_model_to_grid_new(self,
                             grid=None):
        '''
        ...
        '''
        #if not isinstance(grid, radmc_grid):
        #    raise TypeError('grid must be an instance of radmc_grid class')
...
class Setup:
    ...
    def write_dust_density_file(self):
    ...
    def write_temperature_file(self):
        ...
        self.T_avg = np.sum(self.model.T, axis=-1)/self.dust_spec
    ...
    def write_gas_density_file(self,
                                abundance   = 1e-10,
                                snowline    = None,
                                enhancement = 1e5,
                                gas_inside_rcb = True,
      ...
      with open('numberdens_ch3oh.inp','w+') as f:
        f.write('1\n')                       # Format number
        f.write('%d\n'%(self.grid.n1*self.grid.n2*self.grid.n3))           # Nr of cells
        data = nch3oh.ravel(order='F')          # Create a 1-D view, fortran-style indexing
        data.tofile(f, sep='\n', format="%13.6e")
        f.write('\n')

    def write_gas_velocity_file(self, Kep = True,
                          vinfall_per_kep = None,
                                      Rcb = None,
                                  outflow = None
                        ):
      '''
      ...
      '''
      if vinfall_per_kep is None: 
        vinfall = 0
      else:
        vinfall = vinfall_per_kep
      
      self.rcb = Rcb
      if Rcb is None or Rcb == 0:
        vr        = -vinfall*np.sqrt(G*self.model.Mstar*Msun/(self.model.DM.r_sph*au)) # Infall velocity
        vtheta    = 0  # No vertical movement in this version
        if Kep is True:
          vphi    = np.sqrt(G*self.model.Mstar*Msun/(self.model.DM.r_sph*au))    # Keplerian velocity
        elif Kep is False:
          vphi    = np.zeros(vr.shape)
      elif Rcb is not None:  # velocity field in Oya et al. 2014
        rcb_idx   = np.searchsorted(self.model.DM.r_sph, Rcb)
        if outflow is None:
          vr_inside    = np.zeros(self.model.DM.r_sph[:rcb_idx].shape)  # no infall compoonent inside the centrifugal barrier
        elif outflow is not None:
          vr_inside    = +outflow*np.sqrt(G*self.model.Mstar*Msun/(self.model.DM.r_sph[:rcb_idx]*au))  # if expanding
        vr_infall = -vinfall*np.sqrt(G*self.model.Mstar*Msun/(self.model.DM.r_sph[rcb_idx:]*au))
        vr        = np.concatenate((vr_inside, vr_infall))
        vtheta    = 0
        vphi      = np.sqrt(G*self.model.Mstar*Msun/(self.model.DM.r_sph*au))

      with open('gas_velocity.inp','w+') as f:
        f.write('1\n')
        f.write('%d\n'%(self.grid.n1*self.grid.n2*self.grid.n3))
        for idx_phi in range(self.grid.n3):
          for idx_theta in range(self.grid.n2):
            for idx_r in range(self.grid.n1):
              f.write('%13.6e %13.6e %13.6e \n'%(vr[idx_r],vtheta,vphi[idx_r]))

    def get_radiationheat(self, L_star = None,
                                R_star = None,
                           irradiation = True,
                      ):
      '''
      ...
      if irradiation is True:# Irradiation heating calculated by RADMC-3D
        ...
        os.system('rm -rf dust_temperature.dat')
